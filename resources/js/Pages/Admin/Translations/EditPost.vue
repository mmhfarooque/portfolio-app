...
const submit = () => {
    form.post(route('admin.translations.post.update', props.post.id));
};
...
const isLargeTextarea = (field) => {
    return field === 'content';
};
</script>

<template>
    <Head :title="`Translate: ${post.title}`" />
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-gray-900">{{ post.title }}</p>
                            </div>
                            <div v-if="post.excerpt">
                                <InputLabel value="Excerpt" />
                                <p class="mt-1 text-gray-900">{{ post.excerpt }}</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div v-for="(name, code) in locales" :key="code" v-show="activeTab === code && code !== 'en'">
                                <div class="space-y-4">
                                    <div v-for="field in fields" :key="field">
                                        ...
                                        <textarea v-if="isLargeTextarea(field)" v-model="form.translations[code][field]" rows="15" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm font-mono text-sm"></textarea>
                                        <textarea v-else-if="isTextarea(field)" v-model="form.translations[code][field]" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"></textarea>
