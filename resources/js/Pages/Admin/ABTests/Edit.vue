...
const form = useForm({
    name: props.abtest.name,
    description: props.abtest.description || '',
    sample_size: props.abtest.sample_size,
    confidence_level: props.abtest.confidence_level,
    variants: props.abtest.variants || []
});
...
const submit = () => {
    form.put(route('admin.abtests.update', props.abtest.id));
};
</script>

<template>
    <Head :title="`Edit: ${abtest.name}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center">
                <Link :href="route('admin.abtests.show', abtest.id)" class="text-gray-500 hover:text-gray-700 mr-4">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </Link>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit A/B Test</h2>
            </div>
        </template>
            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="description" v-model="form.description" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"></textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div v-for="(variant, index) in form.variants" :key="index" class="border border-gray-200 rounded-lg p-4">
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <TextInput v-model="variant.value" class="w-full text-sm" required />
                                    </div>
                                    <div>
                    ...
                    <div class="flex justify-end gap-3">
                        <Link :href="route('admin.abtests.show', abtest.id)">
