...
<template>
    <Head :title="`SEO Audit - ${photo.title}`" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Photo SEO Audit</h2>
            </div>
        </template>
                    ...
                    <div class="flex gap-6">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between">
                                <div>
                                    <h3 class="text-xl font-semibold text-gray-900">{{ photo.title }}</h3>
                                    <p class="text-sm text-gray-500 mt-1">{{ photo.slug }}</p>
                                </div>
                    ...
                    <dl class="space-y-4">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ photo.seo_title || '(Not set - using photo title)' }}
                                <span class="text-gray-400 ml-2">({{ (photo.seo_title || photo.title).length }}/60 chars)</span>
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Meta Description</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ photo.meta_description || '(Not set - using description)' }}
                                <span class="text-gray-400 ml-2">({{ (photo.meta_description || photo.description || '').length }}/160 chars)</span>
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Category</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ photo.category?.name || '(Not set)' }}
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Tags</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                <span v-if="photo.tags && photo.tags.length > 0" class="flex flex-wrap gap-1">
                                    <span v-for="tag in photo.tags" :key="tag.id" class="px-2 py-0.5 bg-gray-100 rounded-full text-xs">
                                        {{ tag.name }}
                                    </span>
                                </span>
