...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <!-- Fonts - Photography Theme Fonts -->
        <link rel="preconnect" href="https://fonts.bunny.net">
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <!-- Darkroom theme: Plus Jakarta Sans + Sora -->
        <!-- Gallery theme: Crimson Pro + Cormorant Garamond -->
        <!-- Vintage theme: Libre Baskerville + Playfair Display -->
        <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400&family=Crimson+Pro:ital,wght@0,400;0,500;0,600;1,400&family=Libre+Baskerville:ital,wght@0,400;0,700;1,400&family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400&family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Sora:wght@400;500;600;700&display=swap" rel="stylesheet">
    ...
    <body class="font-sans antialiased min-h-screen bg-theme-primary text-theme-primary">
        ...
        <!-- Footer -->
        <footer class="mt-20 border-t border-theme/30">
            <div class="max-w-screen-xl mx-auto px-6 sm:px-8 lg:px-12 py-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    <!-- Left: Copyright -->
                    <p class="text-sm text-theme-muted/60 order-2 md:order-1">
                        &copy; {{ date('Y') }} {{ App\Models\Setting::get('profile_name', App\Models\Setting::get('site_name', 'Photography Portfolio')) }}
                    </p>

                    <!-- Center: Navigation -->
                    <nav class="flex items-center gap-6 order-1 md:order-2">
                        <a href="{{ route('home') }}" class="text-sm text-theme-muted/70 hover:text-theme-primary transition-colors">Home</a>
                        <a href="{{ route('photos.index') }}" class="text-sm text-theme-muted/70 hover:text-theme-primary transition-colors">Gallery</a>
                        <a href="{{ route('blog.index') }}" class="text-sm text-theme-muted/70 hover:text-theme-primary transition-colors">Blog</a>
                        <a href="{{ route('contact') }}" class="text-sm text-theme-muted/70 hover:text-theme-primary transition-colors">Contact</a>
                    </nav>

                    <!-- Right: Social Icons -->
                    <div class="flex items-center gap-4 order-3">
                        @if (App\Models\Setting::get('social_instagram'))
                            <a href="{{ App\Models\Setting::get('social_instagram') }}" target="_blank" rel="noopener noreferrer" class="text-theme-muted/50 hover:text-theme-primary transition-colors" title="Instagram">
                        ...
                        @if (App\Models\Setting::get('social_twitter'))
                            <a href="{{ App\Models\Setting::get('social_twitter') }}" target="_blank" rel="noopener noreferrer" class="text-theme-muted/50 hover:text-theme-primary transition-colors" title="X">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                            </a>
                        @endif
                        @if (App\Models\Setting::get('social_linkedin'))
                            <a href="{{ App\Models\Setting::get('social_linkedin') }}" target="_blank" rel="noopener noreferrer" class="text-theme-muted/50 hover:text-theme-primary transition-colors" title="LinkedIn">
                        ...
                        @if (App\Models\Setting::get('social_github'))
                            <a href="{{ App\Models\Setting::get('social_github') }}" target="_blank" rel="noopener noreferrer" class="text-theme-muted/50 hover:text-theme-primary transition-colors" title="GitHub">
